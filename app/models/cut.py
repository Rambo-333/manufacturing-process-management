...
class NewCut(BaseDatabase):
    __tablename__ = "cutnew"
    ...
    weight = Column(Float)
    memo = Column(String)
    TopList = Column(String)
    BtmList = Column(String)
    SV_len = Column(Integer)
    Top_len = Column(Integer)
    Btm_len = Column(Integer)
    Top_OD = Column(Float)
    Btm_OD = Column(Float)
    Min_OD = Column(Float)
    Max_OD = Column(Float)
    ...
    def read_data():
        ...
        dbdata = session.query(NewCut).order_by(NewCut.id.desc()).limit(num).all()
        data = []
