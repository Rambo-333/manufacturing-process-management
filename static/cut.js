...
const list = [
    {val:"",txt:"--選択--"},
    {val:"内径膨れ",txt:"1.内径膨れ"},
    {val:"リングずれ",txt:"2.リングずれ"},
    {val:"外径細い",txt:"3.外径細い"},
    {val:"溶残",txt:"4.溶残"},
    {val:"周状アバタ",txt:"5.周状アバタ"},
    {val:"指定",txt:"6.指定"},
    {val:"その他",txt:"7.その他"},]
...
window.onload = () => {
    //ドロップダウンリスト
    let tmpT = document.getElementById("TopList");
    let tmpB = document.getElementById("BtmList");
    pullDownRun(tmpT);
    pullDownRun(tmpB);
    //作業者
    workers();
...
//下部カット長計算
function calculate(){
    let x1 = document.getElementById("SV_len").value;
    let x2 = document.getElementById("Top_len").value;
    let x3 = document.getElementById("length").value;
    var answer = x1-x2-x3;
    if(answer<0 || x1 == '' || x2 == '' || x3 == ''){
        answer = '';
    }
    document.getElementById("Btm_len").value = answer;
}
...
async function checkDB(noCheck){
    try {
        const response = await fetch(`/cut?input=${noCheck}`,{
            method: 'GET'
        });
        if (response.ok){
...
kind.addEventListener('change', () => {
    ...
    setAttributes(".infoOVD", kindTop !== "R");
    setAttributes(".infoVAD", kindTop === "R");
});
